...
for seq in tqdm(listdir(mask_dir)):
        ...
        for f in listdir(seq_mask_dir):
            ...
            max_pix = np.max(mask)
            mask = np.uint8(mask/max_pix * 255.0)
            ...
            U = unary_from_softmax(labels,scale=1)
            ...
            feats = create_pairwise_bilateral(sdims=(25, 25), schan=(5, 5, 5),
                                              img=image, chdim=2)
            crf.addPairwiseEnergy(feats, compat=7,
                                  kernel=dcrf.DIAG_KERNEL,
                                  normalization=dcrf.NORMALIZE_SYMMETRIC)
